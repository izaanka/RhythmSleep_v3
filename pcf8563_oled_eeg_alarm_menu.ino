/*
  ...
  ===================================================================
  FEATURES:
  1. Universal USB-C DAC Soundcard Audio Engine:
     - Uses C++17 __has_include(<usb/usb_host.h>) check so it NEVER fails compilation
       regardless of USB Host settings in the Arduino IDE Tools menu.
     - Automatically uses ESP32-S3 hardware USB OTG Host stack when available, or
       high-precision 100 Hz differential DAC drive across USB D+ (GPIO 20) & D- (GPIO 19).
  2. Ultra-Reliable SD Card Module Mount (GPIO 10 CS):
     - Initialized first before display drivers to avoid SPI bus lock.
     - Logs exact status, card format (SDHC/SDSC), and size in MB.
  3. Touch Code Removed: Pure hardware SPI for TFT Display & SD Card.
     - Visual bottom button labels ([MENU], [UP+], [DN-], [SELECT]) retained.
  4. Serial Monitor Time Sync: Set RTC date & time by sending string:
     "dd/mm/yyyy-hh:mm:ss" (e.g. "01/08/2026-21:53:00").
  5. Smart EEG Alarm: Triggers within user [Start..End] window when user 
     is in Lightest Sleep for at least 2 minutes (>50% average certainty).
  6. Rapid Haptic Vibration: Pulses GPIO 21 vibration motor when alarm rings.
  7. Strict Dismissal: Alarm can ONLY be turned OFF by the OK (SELECT) button.
  8. 2.8" ST7789 TFT Display + 0.96" SSD1306 OLED Support.
  9. 1-Minute Inactivity Backlight Sleep Timeout (GPIO 48 BLK Control).
 10. RhythmSleep 16->32->16->4 MLP Neural Network AI Model.
  ===================================================================
*/
...
// Safe C++17 check for ESP-IDF USB Host Subsystem
#if __has_include(<usb/usb_host.h>)
  #include <usb/usb_host.h>
  #define HAS_HARDWARE_USB_HOST 1
#else
  #define HAS_HARDWARE_USB_HOST 0
#endif
